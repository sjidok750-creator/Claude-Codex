...
export function StatusBar({ runner, connected, participants }: Props) {
  ...
  const agent = (label: string, cls: string, st: RunnerStatus['claude'] | undefined, p: Participant | undefined) => {
    ...
    const state = !st.available ? <span className="off">없음</span> : st.loggedIn === false ? <span className="warn">로그인 필요</span> : <span className="on">ready</span>;
    return (
      <span title={st.detail ?? undefined}>
        <span className={cls}>{label}</span> {state}
        {p && <> · {p.model ?? '-'}{p.effort ? `/${p.effort}` : ''}{!p.enabled ? ' · muted' : ''}</>}
      </span>
    );
  };
  return (
    <div className="statusbar">
      <span className={connected ? 'on' : 'off'}>● {connected ? 'hub connected' : 'hub disconnected'}</span>
      {runner && <span>{runner.hostname}{runner.mock ? ' · MOCK' : ''}</span>}
